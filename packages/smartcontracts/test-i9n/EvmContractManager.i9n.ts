...
describe("EvmContractManager", () => {
  ...
  describe("Generic smart contract deployment", () => {
    it("should register and deploy an arbitrary smart contract with no contract deploy args", async () => {
      ...
      await expect(
        hardhatNetwork.ethersRpcProvider.getCode(contract.address)
      ).resolves.not.toStrictEqual("0x");
    });

    it("should return a working deployed contract reference by deployed name", async () => {
      const contract =
        evmContractManager.getDeployedContract<TestContract>("TestContract");
      await expect(contract.resolvedAddress).resolves.toStrictEqual(
        expect.any(String)
      );
    });
    ...
    it("should return all deployed contracts", async () => {
      expect(
        Object.fromEntries(evmContractManager.getDeployedContracts())
      ).toStrictEqual({
        TestContract: {
          name: "TestContract",
          ref: expect.anything(),
    ...
    it("should throw error if attempting to get a contract that has not been deployed yet", async () => {
      await expect(async () =>
        evmContractManager.getDeployedContract<TestContract>("AnotherContract")
      ).rejects.toThrow("Contract 'AnotherContract' has not been deployed yet");
    });
  ...
  describe("isContractDeployed", () => {
    it("should return false if a contract has not yet been successfully deployed to chain", async () => {
      ...
      expect(
        await evmContractManager.isContractDeployedOnChain(
          "UnminedContractDeployment"
        )
      ).toStrictEqual(false);
    });
  });
  ...
  describe("Attaching signer to deployed contracts", () => {
    it("should return a new contract with the otherTestSigner when passed as a param", async () => {
      const contract = evmContractManager.getDeployedContract<TestContract>(
        "TestContract",
        otherTestSigner
      );
      expect(await contract.signer.getAddress()).toStrictEqual(
        otherTestSigner.address
      );
    });
    ...
    it("should not mutate the original contract in the object", async () => {
      ...
      for (const [, contract] of deployedContracts) {
        ...
        expect(await contract.ref.signer.getAddress()).toStrictEqual(
          await hardhatNetwork.contractSigner.getAddress()
        );
      }
    });
  });
