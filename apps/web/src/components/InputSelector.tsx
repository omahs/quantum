...
export function InputSelector({
  ...
  const roundedBorderStyle = type === SelectionType.Network ? 'rounded-l-lg' : 'rounded-r-lg';
  const { name, icon } = type === SelectionType.Network ? (value as NetworkOptionsI) : (value as TokensI).tokenA;
  return (
    <div>
      <span className="text-dark-900 pl-5 text-xs font-semibold lg:text-base">{label}</span>
            ...
            >
              ...
              >
                <div className="flex flex-row items-center">
                  ...
                  />
                  <span className="text-dark-1000 ml-2 block truncate text-sm lg:text-xl">{name}</span>
                </div>
                {!disabled && (
                  <span className="text-dark-900">
                    <FiChevronDown
                      className={clsx('text-dark-900 h-5 w-5 transition-[transform] lg:h-6 lg:w-6', {
                        'rotate-180': open,
                      })}
                    />
                  </span>
              ...
              <div className={clsx('my-1 px-5 py-3 lg:my-2 lg:px-6 lg:py-4', active && 'bg-dark-gradient-1')}>
                <div className="flex flex-row items-center justify-between">
                  <div className="flex w-4/12 flex-row items-center">
                    ...
                    />
                    <span className="text-dark-1000 ml-2 truncate text-base lg:text-lg">{option.tokenA.name}</span>
                  </div>
                  ...
                  <div className="flex w-4/12 flex-row items-center">
                    ...
                    />
                    <span className="text-dark-900 ml-2 truncate text-base lg:text-lg">{option.tokenB.name}</span>
                  </div>
